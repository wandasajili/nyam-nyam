...
<head>
  ...
  <style>
    @media (max-width: 767px) {
      ...
      .table-responsive-stack td.td-image {
        padding-left: 0;
        text-align: center;
        margin-bottom: 10px;
      }
                ...
                <div class="card">
                  <div class="card-body">
                    <div class="row align-items-center mb-3">
                      <div class="col">
                        <h4 class="card-title m-0">Menu</h4>
               
                      </div>
                      <div class="col-auto">
                        <a href="/admin/menu/create" class="btn btn-sm btn-primary">Add Item</a>
                      </div>
                    </div>
                    ...
                    <div class="table-responsive-custom hide-scrollbar">
                      <table class="table table-responsive-stack">
                        ...
                        <tbody>
                          ...
                          <tr>
                            <td class="td-image" data-label="Image">
                              <img src="{{ asset('img/' . $menu->image) }}" alt="{{ $menu->name }}" width="60" height="60" style="border-radius: 0;">
                            </td>
                            <td data-label="Name">{{ $menu->name }}</td>
                            <td data-label="Description" style="max-width: 250px; white-space:normal; word-wrap:break-word">{{ $menu->description }}</td>
                            <td data-label="Price">{{ $menu->price }}</td>
                            <td data-label="Stock">{{ $menu->stock }}</td>
                            <td data-label="Updated At">{{ $menu->updated_at }}</td>
                            <td class="td-actions" data-label="Action">
                              <form action="{{ route('menu.destroy', $menu->id) }}" method="POST" class="action-form">
                                @csrf
                                @method('DELETE')
                                <a href="{{ route('menu.edit', $menu->id) }}" class="btn btn-sm btn-primary">Update</a>
